...
describe('TransactionsService', () => {
  ...
  it('listByWallet: returns txs if owned', async () => {
    jest
      .spyOn(walletRepo, 'findOne')
      .mockResolvedValue({ id: 'w1', user: { id: 'u1' } } as any);
  ...
  it('createAndSend: throws Forbidden if wallet not owned', async () => {
    (dataSource.transaction as jest.Mock).mockImplementation(async (fn) => {
      const manager = {
        findOne: jest
          .fn()
          .mockResolvedValue({ id: 'w1', user: { id: 'other' } }),
        create: jest.fn(),
        save: jest.fn(),
      };
      return fn(manager);
  ...
  it('refreshStatus: throws Forbidden if tx not owned', async () => {
    (dataSource.transaction as jest.Mock).mockImplementation(async (fn) => {
      const manager = {
        findOne: jest
          .fn()
          .mockResolvedValue({ id: 't1', wallet: { user: { id: 'other' } } }),
        save: jest.fn(),
      };
      return fn(manager);
    });

    await expect(service.refreshStatus('t1', 'u1')).rejects.toThrow(
      'Forbidden',
    );
  });
